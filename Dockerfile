...
# Start OTP, download graph if missing, and log info
CMD /bin/bash -c "\
if [ ! -f /var/otp/data/graph.obj ]; then \
    echo 'graph.obj not found. Downloading from GitHub Releases...'; \
    curl -L -o /var/otp/data/graph.obj https://github.com/RuetGG/OTP-Setup-V3/releases/download/v1/graph.obj; \
fi; \
echo 'File sizes in /var/otp/data:'; \
ls -lh /var/otp/data; \
